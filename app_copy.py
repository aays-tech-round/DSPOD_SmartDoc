        ...
        with st.spinner("Analysing the file ..."):
            ...
            if option == "AP Invoice":
                ...
                if "BILL OF ENTRY SUMMARY" in check_text:
                    ...
                    # st.dataframe(summary)
                    try:
                        invoice['invoice_page_no'] = invoice['subtitle'].apply(
                            lambda x: x.split('Invoice ')[1].split('/')[0])

                        invoice = invoice.drop(['page_title', 'subtitle'], axis=1)
                        duties = duties.drop(['page_title', 'subtitle'], axis=1)

                        merged_df = pd.merge(summary, invoice, left_on='invoice_number',
                                             right_on='invoice_invoice_number', how='left', suffixes=('', '_duplicate'))
                        merged_df = pd.merge(merged_df, duties, left_on='invoice_page_no', right_on='duties_invsno',
                        ...
                        csv = merged_df.to_csv(index=False)
                        csv_as_bytes = StringIO(csv).getvalue().encode('utf-8')
                        st.download_button(label="Download a CSV", data=csv_as_bytes, file_name='data.csv',
                                           mime='text/csv')
                        ...
                        merged_df = pd.merge(summary, invoice, left_on='invoice_number',
                                             right_on='invoice_invoice_number', how='left', suffixes=('', '_duplicate'))
                        merged_df = pd.merge(merged_df, duties, left_on='invoice_s.no', right_on='duties_invsno',
                                             how='left', suffixes=('', '_duplicate'))
                        # merged_df=merged_df.drop(['duties_acd_amount','duties_sws_amount','duties_sad_amount','duties_g.cess_amount'],axis=1)
                        # st.write(cache_boes)
